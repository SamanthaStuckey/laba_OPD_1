...
public class EnemyMovementManager : MonoBehaviour
{
    ...
    private IEnumerator DelayLoop()
    {
       for (int i = 0; i < 10; i++)
       {
            ...
            {
                if (GridManager.instance.enemyUnits[i].type == UnitType.Range && GridManager.instance.enemyUnits[i].alive)
                {
                ...
                else if (GridManager.instance.enemyUnits[i].type == UnitType.Melee && GridManager.instance.enemyUnits[i].alive)
                {
                    BaseUnit currentEnemy = GridManager.instance.enemyUnits[i];
                    //RiflemanBrain(i);
                    //yield return new WaitForSeconds(2);
    ...
    {
        distanceToTarget = Mathf.Pow(actingUnit.currentTile.xPos - target.currentTile.xPos, 2) + Mathf.Pow(actingUnit.currentTile.yPos - target.currentTile.yPos, 2);
    ...
    public void EnemyShot(BaseUnit target, BaseUnit actingUnit)
    {
        ...
        if (toHitRoll >= actingUnit.balistikSkill)
        {
            ...
            else target.animator.SetTrigger("GetDamage");
        }
        else
        {
            Debug.Log("miss hit");
    ...
    public bool CombatCheck(BaseUnit checkingUnit)
    {
        ...
        for (int i = ii; i <= iii; i++)
        {
            ...
            {
                if (GridManager.instance.tiles[i + checkingUnit.currentTile.xPos, j + checkingUnit.currentTile.yPos].unit != null)
                {
                    if (GridManager.instance.tiles[i + checkingUnit.currentTile.xPos, j + checkingUnit.currentTile.yPos].unit.isAlly == true)
                    {
            ...
            for (int j = jj; j < jjj; j++)
            {
                if (GridManager.instance.tiles[i + checkingUnit.currentTile.xPos, j + checkingUnit.currentTile.yPos].unit != null)
                {
                    if (GridManager.instance.tiles[i + checkingUnit.currentTile.xPos, j + checkingUnit.currentTile.yPos].unit.isAlly == true)
                    {
                        meleeTargetX = i + checkingUnit.currentTile.xPos;
                        meleeTargetY = j + checkingUnit.currentTile.yPos;
                        return GridManager.instance.tiles[i + checkingUnit.currentTile.xPos, j + checkingUnit.currentTile.yPos].unit;
    ...
    public BaseUnit WatchingClosestTarget(BaseUnit aimingUnit)
    {
        ...
        for(int j = 0; j < 10; j++)
        {
            if (GridManager.instance.playerUnits[j] != null && GridManager.instance.playerUnits[j].alive)
            {
                potentialTarget = GridManager.instance.playerUnits[j];
                distanceToTarget = Mathf.Pow(aimingUnit.currentTile.xPos - potentialTarget.currentTile.xPos, 2) + Mathf.Pow(aimingUnit.currentTile.yPos - potentialTarget.currentTile.yPos, 2);
                if(closestTarget == null)
                {
    ...
    {
        return Mathf.Pow(aimingUnit.currentTile.xPos - closestTarget.currentTile.xPos, 2) + Mathf.Pow(aimingUnit.currentTile.yPos - closestTarget.currentTile.yPos, 2);
    }
}